...
class Oads2sXmlService {
    ...
    def translateSpreadsheet(InputStream inputStream, Charset charset) {            // TODO: should move this elsewhere
        OadsMetadataDocumentType xmlDoc =
                new Excel2OAP(charset).convertToOADS_doc(inputStream)
        return buildDocumentFromSocatVersionMetadata(xmlDoc)
    }

    def buildDocumentFromSocatVersionMetadata(OadsMetadataDocumentType xmlMetadata) {
        Document mdDoc = new Document()

        // Investigators
        for (PersonType person : xmlMetadata.getInvestigators()) {
            mdDoc.addToInvestigators((Investigator)fillPersonDomain(person, new Investigator()))
        }
        // Data Submitter
        mdDoc.setDataSubmitter(fillPersonDomain(xmlMetadata.getDataSubmitter(), new DataSubmitter()) as DataSubmitter)
...
//        citation.setDataUse(metadata.getDataUse())
        ...
        def cruiseId = ""
        for (TypedIdentifierType cruise : metadata.getCruiseIds()) {
            cruiseId += cruise.value + " "
        }
    ...
    def fillTimeAndLocDomain(OadsMetadataDocumentType metadata) {
        ...
        GeospatialExtentsType spatialExtents = metadata.getSpatialExtents()
        if ( spatialExtents != null ) {
            def north, east, south, west
            if ( spatialExtents.bounds ) {
                north = spatialExtents.bounds.northernBounds
                east = spatialExtents.bounds.easternBounds
                south = spatialExtents.bounds.southernBounds
                west = spatialExtents.bounds.westernBounds
            } else if ( spatialExtents.location ) {
                north = south = spatialExtents.location.latitude
                east = west = spatialExtents.location.longitude
            } else {
                north = south = east = west = ""
            }
            timeAndLocation.setWestLon(nonNullString(west))
            timeAndLocation.setEastLon(nonNullString(east))
            timeAndLocation.setNorthLat(nonNullString(north))
            timeAndLocation.setSouthLat(nonNullString(south))
...
//        if ( ! isEmpty(detailedInfo) ) {
        v.setDetailedInformation(source.getDetailedAnalyzingInfo())
//        }
...
//                }
    ...
    private DicVariableType fillDic(GenericVariable v) {
        DicVariableTypeBuilder dicVarBuilder = (DicVariableTypeBuilder) _fillDic(v, DicVariableType.builder())
        dicVarBuilder.name("disolved_inorganic_carbon")
        return dicVarBuilder.build()
    }
    private DicVariableTypeBuilder _fillDic(GenericVariable v, DicVariableTypeBuilder builder) {
        DicVariableTypeBuilder dicVarBuilder = (DicVariableTypeBuilder) fillVariable(v, builder)
        if ( v.getPoison()) {
...
//        ph.standardization.phOfStandards(v.getPhStandards())
    ...
    private Co2Autonomous fillPCO2a(GenericVariable v) {
        ...
        */
        ...
        co2Builder.co2GasDryingMethod(v.getDryingMethod())
